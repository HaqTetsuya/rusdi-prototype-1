...
def main():
    """
    Main program loop for processing user input and recognizing intents.
    Continuously takes user input, preprocesses it, recognizes intent, matches courses or days.
    """
    # Load data from JSON
    data = load_data('data.json')
    intents = data['intents']
    courses = data['courses']
    days = [day.lower() for day in data['day']]  # Load day data

    # Define relevant intents for course and day matching
    ...
    debug_message(f"Loaded intents: {list(intents.keys())}")
    debug_message(f"Loaded courses: {courses}")
    debug_message(f"Loaded days of the week: {days}\n")
    ...
    while True:
        ...
        # If the recognized intent is relevant for course matching
        if intent in relevant_course_intents:
            matched_courses = match_course(course_tokens, courses)
            print(f"Intent terdeteksi: {intent}")
            print(f"Mata kuliah terdeteksi: {matched_courses}\n")

        # If the recognized intent is relevant for day matching
        elif intent in relevant_day_intents:
            matched_day = match_day(tokens, days)
            print(f"Intent terdeteksi: {intent}")
            print(f"Hari terdeteksi: {matched_day}\n")

        else:
            print(f"Intent terdeteksi: {intent}")
            print(f"Intent {intent} tidak relevan untuk pencocokan mata kuliah atau hari.\n")
